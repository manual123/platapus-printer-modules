...
class _PosixPipeAddress(_AbstractPipeAddress):
    def listen(self):
        ...
        listener = kaiten.listener.PipeListener(self._path, s)
        return listener
    ...
    def connect(self):
        ...
        connection = kaiten.connection.SocketConnection(s, None)
        return connection
...
class TcpAddress(Address):
    ...
    def get_socket(self):
        return self.socket_factory(self._port, self._host)

    @staticmethod
    def socket_factory(port, host='localhost'):
    ...
    def listener_factory(port, host='localhost'):
        ...
        s.listen(socket.SOMAXCONN)
        #s.setblocking(0)
        listener = kaiten.listener.TcpListener(s)
        return listener
    ...
    def connect(self):
        ...
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.setblocking(0)
        s.connect_ex((self._host, self._port))
        connection = kaiten.connection.SocketConnection(s, None)
        return connection
